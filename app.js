(function () {
  ...
  /* ---- Horario del comedor (configurable desde Configuración) ---- */
  const HORA_DESDE_DEFECTO = "12:00";
  const HORA_HASTA_DEFECTO = "13:00";
  // "12:30" -> 750 minutos desde la medianoche, para poder comparar horarios.
  ...
  function horarioComedor() {
    const desde = state.config.horaDesde || HORA_DESDE_DEFECTO;
    const hasta = state.config.horaHasta || HORA_HASTA_DEFECTO;
    return { desde: desde, hasta: hasta, desdeMin: aMinutos(desde), hastaMin: aMinutos(hasta) };
  }
  function minutosAhora(d) {
    const n = d || new Date();
    return n.getHours() * 60 + n.getMinutes();
  }
  // Pasó el horario: es cuando tiene sentido reclamar a los que no volvieron.
  function pasoElHorario(d) {
    return minutosAhora(d) >= horarioComedor().hastaMin;
  }
  ...
  const state = {
    ...
    dayLoaded: false,
    sonido: false,            // pitido al registrar (ajuste de este dispositivo)
    cursoFilter: "",          // "" = todos los cursos
    kiosk: false,
    micContinuo: false,
    config: {},               // { whatsapp, email } — contacto de preceptoría
    auditList: [],
    alertAvisadaDia: null,    // para avisar una sola vez por día al pasar el horario
  };
  ...
  function tickClock() {
    ...
    const h = horarioComedor();
    const ahora = minutosAhora(now);
    const badge = document.getElementById("horarioBadge");
    if (ahora >= h.desdeMin && ahora < h.hastaMin) {
      badge.className = "badge good";
      badge.innerHTML = '<span class="badge-dot"></span>Horario de comedor (' + escapeHtml(h.desde) + '–' + escapeHtml(h.hasta) + ')';
    } else {
  ...
  // Configuración la ve todo el mundo: adentro tiene los ajustes de su propio
  // dispositivo (tema, letra, sonido). Las secciones de administración se
  // ocultan por separado, en applyConfigVisibility().
  const ROLE_TABS = {
    director: ["comedor", "alumnos", "reportes", "config"],
    preceptor: ["comedor", "config"],
    secretaria: ["reportes", "config"],
  };
  ...
  function applyRoleUI(role) {
    ...
    const map = {
      ...
      config: document.getElementById("tabBtnConfig"),
    };
    ...
    Object.keys(map).forEach((key) => {
      ...
      btn.hidden = !ok;
      // Configuración no puede ser la pestaña de arranque: si el rol solo tuviera
      // esa, quedaría mirando ajustes en vez de su trabajo.
      if (ok && key !== "config" && !firstAllowed) firstAllowed = key;
    });
    applyConfigVisibility();
    setTab(firstAllowed || "config");
  }

  // Muestra u oculta las secciones de administración dentro de Configuración.
  // Sin login (modo local) se ven todas.
  function applyConfigVisibility() {
    const esAdmin = !state.currentRole || state.currentRole === "director";
    document.querySelectorAll(".cfg-admin").forEach((el) => { el.hidden = !esAdmin; });
    const panelCuenta = document.getElementById("panelCuenta");
    if (panelCuenta) panelCuenta.hidden = !state.currentEmail;
    if (state.currentEmail) {
      document.getElementById("cuentaEmail").textContent = state.currentEmail;
      document.getElementById("cuentaRol").textContent = ROLE_LABELS[state.currentRole] || "";
    }
  }
  ...
  function startBackend(b) {
    ...
    if (backend.watchConfig) {
      unsubConfig = backend.watchConfig((cfg) => {
        state.config = cfg || {};
        renderConfig();
        tickClock();        // el horario del comedor puede haber cambiado
        renderRetiroList();
      });
    }
  ...
  function renderRetiroList() {
    ...
    const vueltosIds = new Set(state.dayEvents.map((e) => e.studentId));
    const now = new Date();
    const pasadoHorario = pasoElHorario(now);
  ...
  const UMBRALES_VOZ = {
    cauto:       { minimo: 0.75, margen: 0.26 },
    equilibrado: { minimo: 0.65, margen: 0.18 },
    agil:        { minimo: 0.55, margen: 0.10 },
  };
  ...
  function handleTranscripts(alternatives) {
    ...
    // Umbrales según la sensibilidad elegida en Configuración: "minimo" es qué tan
    // parecido tiene que ser el nombre, y "margen" cuánta ventaja tiene que sacarle
    // al segundo candidato para registrarlo sin preguntar.
    const u = UMBRALES_VOZ[state.config.voz] || UMBRALES_VOZ.equilibrado;
    const esClaro = top.score >= u.minimo && (!second || top.score - second.score >= u.margen);
    if (esClaro) {
  ...
  /* ---- Ajustes de este dispositivo (localStorage, no se comparten) ---- */
  const LS_TEMA = "comedorFrench:tema";
  const LS_TAMANO = "comedorFrench:tamano";
  const LS_SONIDO = "comedorFrench:sonido";

  function leerLS(clave, porDefecto) {
    try { const v = localStorage.getItem(clave); return v === null ? porDefecto : v; }
    catch (e) { return porDefecto; }
  }
  function guardarLS(clave, valor) {
    try { localStorage.setItem(clave, valor); } catch (e) { /* storage bloqueado */ }
  }
  ...
  function aplicarTema(tema) {
    ...
    else delete raiz.dataset.theme;          // automático: manda el dispositivo
    guardarLS(LS_TEMA, tema);
    marcarSeg("segTema", "tema", tema);
  }
  function aplicarTamano(tam) {
    const raiz = document.documentElement;
    if (tam === "grande" || tam === "enorme") raiz.dataset.size = tam;
    else delete raiz.dataset.size;
    guardarLS(LS_TAMANO, tam);
    marcarSeg("segTamano", "tamano", tam);
  }
  function aplicarSonido(activo) {
    state.sonido = !!activo;
    guardarLS(LS_SONIDO, activo ? "1" : "0");
    marcarSeg("segSonido", "sonido", activo ? "1" : "0");
  }
  function marcarSeg(idGrupo, attr, valor) {
    const grupo = document.getElementById(idGrupo);
    if (!grupo) return;
    grupo.querySelectorAll("[data-" + attr + "]").forEach((b) => {
      b.setAttribute("aria-pressed", String(b.dataset[attr] === valor));
    });
  }

  /* ---- Pitido al registrar ---- */
  // Los navegadores no dejan sonar nada hasta que la persona tocó algo en la
  // página. Preparamos el audio en el primer toque y lo reusamos, así el pitido
  // también funciona cuando el registro vino por voz (que no cuenta como toque).
  let audioCtx = null;
  ...
  function pitido() {
    ...
    try {
      ...
      vol.gain.exponentialRampToValueAtTime(0.25, t + 0.01);
      vol.gain.exponentialRampToValueAtTime(0.0001, t + 0.22);
      osc.start(t); osc.stop(t + 0.23);
    } catch (e) { /* si el navegador lo bloquea, seguimos sin sonido */ }
  }

  document.getElementById("segTema").addEventListener("click", (ev) => {
    const b = ev.target.closest("[data-tema]");
    if (b) aplicarTema(b.dataset.tema);
  });
  document.getElementById("segTamano").addEventListener("click", (ev) => {
    const b = ev.target.closest("[data-tamano]");
    if (b) aplicarTamano(b.dataset.tamano);
  });
  document.getElementById("segSonido").addEventListener("click", (ev) => {
    const b = ev.target.closest("[data-sonido]");
    if (b) aplicarSonido(b.dataset.sonido === "1");
  });
  document.getElementById("btnProbarSonido").addEventListener("click", () => {
    if (!state.sonido) { toast("Activá el sonido primero.", "warning"); return; }
    pitido();
  });

  // Estado inicial (el tema y el tamaño ya se aplicaron en el index para que no
  // parpadee; acá solo marcamos los botones y levantamos el sonido).
  aplicarTema(leerLS(LS_TEMA, "auto"));
  aplicarTamano(leerLS(LS_TAMANO, "normal"));
  aplicarSonido(leerLS(LS_SONIDO, "0") === "1");

  /* ---- Mi cuenta ---- */
  document.getElementById("btnCfgCambiarClave").addEventListener("click", () => {
    document.getElementById("btnChangePass").click();
  });
  document.getElementById("btnCfgSalir").addEventListener("click", () => {
    document.getElementById("btnLogout").click();
  });

  /* ---- Ajustes del colegio (compartidos, solo Director) ---- */
  function renderConfig() {
    const campos = [
      ["cfgWhatsapp", state.config.whatsapp || ""],
      ["cfgEmail", state.config.email || ""],
      ["cfgHoraDesde", state.config.horaDesde || HORA_DESDE_DEFECTO],
      ["cfgHoraHasta", state.config.horaHasta || HORA_HASTA_DEFECTO],
      ["cfgVoz", state.config.voz || "equilibrado"],
    ];
    campos.forEach(([id, valor]) => {
      const el = document.getElementById(id);
      // No pisamos lo que la persona está tipeando en ese momento.
  ...
  document.getElementById("btnSaveConfig").addEventListener("click", async () => {
    ...
    const horaDesde = document.getElementById("cfgHoraDesde").value || HORA_DESDE_DEFECTO;
    const horaHasta = document.getElementById("cfgHoraHasta").value || HORA_HASTA_DEFECTO;
    const voz = document.getElementById("cfgVoz").value;
    if (email && email.indexOf("@") === -1) { toast("Ese mail no parece válido.", "critical"); return; }
    if (aMinutos(horaHasta) <= aMinutos(horaDesde)) {
      toast("La hora de fin del comedor tiene que ser posterior a la de inicio.", "critical");
      return;
    }
    const ok = await backend.saveConfig({
      whatsapp: whatsapp, email: email, horaDesde: horaDesde, horaHasta: horaHasta, voz: voz,
    });
    if (ok) {
      audit("Cambio de ajustes del colegio",
        "comedor " + horaDesde + "–" + horaHasta + ", voz " + voz +
        ", aviso a " + (whatsapp || "sin WhatsApp") + " / " + (email || "sin mail"));
      toast("Ajustes guardados para todos los dispositivos.");
      tickClock();
    }
  });
  ...
  document.getElementById("btnBackup").addEventListener("click", async () => {
    ...
    if (!window.XLSX) { toast("No se pudo cargar el generador de Excel.", "critical"); return; }
    if (!backend) return;
    estado.textContent = "Preparando…";
    try {
      const alumnos = Array.from(state.students.values())
        .sort((a, b) => (a.apellido + a.nombre).localeCompare(b.apellido + b.nombre, "es"));
      // Rango bien ancho: las fechas son texto AAAA-MM-DD, así que comparar
      // alfabéticamente alcanza para traer todo lo que haya.
      const eventos = await backend.fetchRange("0000-01-01", "9999-12-31");

      const wb = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(
        alumnos.map((s) => ({
          Apellido: s.apellido, Nombre: s.nombre, Curso: s.curso || "",
          "Retira a comedor": s.retiraComedor ? "si" : "no",
        }))
      ), "Alumnos");
      XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(
        eventos.slice().sort((a, b) => a.ts - b.ts).map((e) => ({
          Fecha: e.dia || "", Hora: e.hora || "", Apellido: e.apellido, Nombre: e.nombre,
          Curso: e.curso || "", "Método": e.metodo === "voz" ? "Voz" : "Manual",
          "Registrado por": e.registradoPor || "",
        }))
      ...
      XLSX.writeFile(wb, "respaldo-comedor-french_" + diaStr(new Date()) + ".xlsx");
      estado.textContent = alumnos.length + " alumnos y " + eventos.length + " reingresos.";
      audit("Descarga de copia de seguridad", alumnos.length + " alumnos, " + eventos.length + " reingresos");
      toast("Copia de seguridad generada.");
    } catch (e) {
      console.error(e);
      estado.textContent = "";
      toast("No se pudo generar la copia de seguridad.", "critical");
    }
  });
